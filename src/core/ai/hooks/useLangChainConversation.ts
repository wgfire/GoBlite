...
import { conversationIdsAtom, currentConversationIdAtom, conversationsAtom, userInputAtom } from "../atoms/conversationAtoms";
...
export function useLangChainConversation() {
  ...
  const addMessage = useCallback(
    (id: string, message: Omit<Message, "id" | "timestamp">) => {
      try {
        if (!conversations[id]) {
          throw new Error(`对话不存在: ${id}`);
        }
  ...
  const clearMessages = useCallback(
    (id: string, keepSystemPrompt: boolean = true) => {
      try {
        if (!conversations[id]) {
          throw new Error(`对话不存在: ${id}`);
        }
