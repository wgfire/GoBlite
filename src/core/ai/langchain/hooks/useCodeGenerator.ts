...
export const useCodeGenerator = (options: UseCodeGeneratorOptions) => {
  ...
  const generateCode = useCallback(
    async (prompt: string, codeOptions?: CodeGenerationOptions): Promise<CodeGenerationResult> => {
      try {
        // 检查服务状态
   ...
   * @returns 是否同步成功
   */
  const syncCodeToFileSystem = useCallback(
    async (files: GeneratedFile[] = generatedFiles, overwrite: boolean = false): Promise<boolean> => {
      try {
        // 检查文件系统
        if (!fileSystem) {
   ...
   * @returns 是否预览成功
   */
  const previewGeneratedCode = useCallback(
    async (files: GeneratedFile[] = generatedFiles): Promise<boolean> => {
      try {
        // 检查WebContainer
        if (!webContainer) {
