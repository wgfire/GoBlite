import { ElementBox, useI18n, useNode, UserComponent } from "@go-blite/design";
...
import { useSymbolData } from "./hooks/useRequest";
...
export const SymbolCardEdit: UserComponent<Partial<React.PropsWithChildren<MainSymbolProps>>> = props => {
  const {
    id,
    connectors: { connect },
    actions: { setProp }
  ...
  const { run: runSetProp } = useDebounceFn(
    (props: MainSymbolProps) => {
      setProp(props);
    },
    { wait: 500 }
  );
  const { getText } = useI18n(id, [
    "title",
    "supportText",
    "stressText",
    "footerContent",
  ...
  useEffect(() => {
    // 只有当环境变量加载完成后，才执行数据请求
    if (!envLoading) {
      console.log(getText("title"), envData);
      symbolData.run();
    }
  }, [envLoading, envData]);

  return (
    <ElementBox ref={node => node && connect(node)} id={id} data-id={id} style={{ width: "100%" }}>
      <div className="mb-[8px] w-full bg-white px-[16px] py-[20px]">
        ...
        <div className="mb-[10px] w-full rounded-[10px] bg-[#006CF6]/10 px-[10px] py-[12px]">
          <p className="mb-[12px] text-center text-[16px] font-medium text-black">黄金</p>
